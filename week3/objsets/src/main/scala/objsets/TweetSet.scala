...
abstract class TweetSet {
   ...
   */
    def filter(p: Tweet => Boolean): TweetSet =
      filterAcc(p, new Empty: TweetSet)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val left_max = if (left.isEmpty) elem else {
      val l = left.mostRetweeted
      if (l.retweets > elem.retweets) l else elem
    }
    if (right.isEmpty) left_max else {
      val r = right.mostRetweeted
      if (r.retweets > left_max.retweets) r else left_max
    }
  }

  def descendingByRetweet: TweetList = {
    val top_tweet = mostRetweeted
    val new_s = remove(top_tweet)
    new Cons(top_tweet, new_s descendingByRetweet)
  }
...
object Main extends App {
  ...
  GoogleVsApple.trending foreach  println _
}
